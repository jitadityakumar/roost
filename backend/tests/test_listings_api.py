...
def test_patch_listing_rejects_with_blank_reason(client):
    store.create_stub_listing(1, VALID_URL)
    resp = client.patch("/api/listings/1", json={"user_status": "rejected", "comment": "   "})
    assert resp.status_code == 422
...
def test_patch_listing_rejects_with_blank_initials(client):
    ...
    resp = client.patch(
        "/api/listings/1",
        json={"user_status": "rejected", "comment": "Too small", "initials": "  "},
    )
    assert resp.status_code == 422
...
def test_patch_listing_rejects_with_reason_stores_it(client):
    ...
    resp = client.patch(
        "/api/listings/1",
        json={"user_status": "rejected", "comment": "Too small", "initials": "JK"},
    )
    assert resp.status_code == 200
    body = resp.json()
...
def test_patch_listing_moves_to_viewing_with_comment(client):
    ...
    resp = client.patch(
        "/api/listings/1",
        json={"user_status": "viewing", "comment": "Booked for Saturday", "initials": "JK"},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert body["user_status"] == "viewing"
...
def test_patch_listing_moves_to_contacted_without_comment_is_rejected(client):
    store.create_stub_listing(1, VALID_URL)
    resp = client.patch("/api/listings/1", json={"user_status": "contacted"})
    assert resp.status_code == 422


def test_patch_listing_moves_to_contacted_with_comment_stores_it(client):
    store.create_stub_listing(1, VALID_URL)
    resp = client.patch(
        "/api/listings/1",
        json={"user_status": "contacted", "comment": "Emailed the agent", "initials": "JK"},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert body["user_status"] == "contacted"
    contacted_comments = [c for c in body["comments"] if c["comment_type"] == "contacted"]
    assert len(contacted_comments) == 1
...
def test_get_listing_computes_min_walk_minutes_from_stored_walk_durations(client):
    # Issue #92: min_walk_minutes is repointed at nearest_station_candidates
    # (TfL-discovered) rather than station_walk_distances/nearest_stations_raw
    # -- that's now the semantically correct "nearest station" source.
    from app.nearest_stations import store as nearest_stations_store
    from app.standards import store as standards_store

    store.create_stub_listing(1, VALID_URL)
    store.apply_extracted_fields(1, {"latitude": 51.5, "longitude": -0.1})
...
def test_get_listing_min_walk_minutes_excludes_candidates_over_max_walk_minutes(client):
    # get_nearest_stations (issue #92) drops any candidate over
    # MAX_WALK_MINUTES -- min_walk_minutes must only ever be computed from
    # what's left after that filter, not the raw stored duration.
    from app.nearest_stations import store as nearest_stations_store
    ...
    nearest_stations_store.replace_candidates(
        ...
        [
            {
                "stop_point_id": "910GFAR",
                "name": "Far Station",
                "modes": "national-rail",
                "lat": None,
                "lon": None,
                "distance_meters": 1900,
                "walk_distance_meters": 1900,
                "duration_seconds": (MAX_WALK_MINUTES + 5) * 60,
                "computed_at": "2026-09-13T00:00:00+00:00",
            },
        ],
    )
